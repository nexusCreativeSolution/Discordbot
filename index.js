...
client.on('messageCreate', async (message) => {
  ...
  if (mentionedUser) {
    ...
    if (afkData) {
      message.reply(
        `${mentionedUser.username} is currently AFK: ${afkData.reason}`
      );
    }
  }
  ...
  while (user.experience >= experienceForNextLevel) {
    ...
    const userRole =
      levelRoles.find((r) => user.level <= r.level)?.role || 'GOD';
...
client.on('guildMemberAdd', async (member) => {
  ...
  if (welcomeChannel) {
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Hello ${member}, welcome to **${member.guild.name}**! We're glad to have you here.`
      )
      .setColor('#00FF00')
